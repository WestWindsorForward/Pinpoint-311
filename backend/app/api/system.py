...
async def get_github_headers():
    """Get GitHub API headers with optional token from secrets."""
    headers = {"Accept": "application/vnd.github.v3+json"}
    try:
        ...
        async with SessionLocal() as db:
            result = await db.execute(
                select(SystemSecret).where(SystemSecret.key_name == "GITHUB_TOKEN")
            )
            secret = result.scalar_one_or_none()
            if secret and secret.key_value:
                token = decrypt_safe(secret.key_value)
                if token:
...
async def get_current_version(
    ...
    """Get the currently deployed version (git SHA and tag if available)."""
    try:
        # Get current git SHA
        ...
        display = tag if tag else (sha[:7] if sha else "unknown")
        ...
        logger.error(f"Failed to get current version: {e}")
        return {"sha": None, "tag": None, "display": "unknown", "commit_message": None}
...
async def list_releases(
    ...
    """List GitHub releases and recent commits."""
...
@router.get("/releases/{ref}/security")
async def get_release_security(
    ref: str,
    _: User = Depends(get_current_admin)
):
    """Get security check status for a specific ref (tag or commit SHA)."""
    headers = await get_github_headers()
    
    # GitHub Actions workflow names to check
    workflows = {
        "owasp_zap": "OWASP ZAP Security Scan",
        "codeql": "CodeQL",
        "docker_build": "Build and Push Docker",
        ...
        for key, name in workflows.items():
            ...
            try:
                ...
                resp = await client.get(
                    f"{GITHUB_API_BASE}/commits/{ref}/check-runs",
                    headers=headers
                )
                if resp.status_code == 200:
                    runs = resp.json().get("check_runs", [])
                    for run in runs:
                        if name.lower() in run["name"].lower() or key.lower() in run["name"].lower():
                            check["status"] = run["status"]
                            check["conclusion"] = run.get("conclusion")
                            check["run_url"] = run.get("html_url")
                            check["passed"] = run.get("conclusion") == "success"
                            if check["passed"]:
                                passed += 1
                            break
            except Exception as e:
                logger.warning(f"Failed to check {key}: {e}")
...
@router.post("/switch-version")
async def switch_version(
    ref: str = Query(..., description="Git ref (tag or commit SHA) to deploy"),
    _: User = Depends(get_current_admin)
):
    """
    Deploy a specific version by checking out a git ref.
    ...
    try:
        ...
        logger.info(f"[Deploy] Creating backup before switching to {ref}")
        ...
        logger.info(f"[Deploy] Fetching from origin...")
        fetch_result = subprocess.run(
            ["git", "fetch", "origin", "--tags"],
            capture_output=True, text=True, timeout=60
        )
        if fetch_result.returncode != 0:
            logger.warning(f"[Deploy] Fetch warning: {fetch_result.stderr}")
        
        # Step 3: Checkout the ref
        logger.info(f"[Deploy] Checking out {ref}...")
        checkout_result = subprocess.run(
            ["git", "checkout", ref],
        ...
        if checkout_result.returncode != 0:
            raise HTTPException(
                status_code=400,
                detail=f"Failed to checkout {ref}: {checkout_result.stderr}"
            )
        ...
        new_sha = sha_result.stdout.strip()[:7] if sha_result.returncode == 0 else ref[:7]
        
        logger.info(f"[Deploy] Successfully switched to {new_sha}")
        
        return {
            "status": "success",
            "message": f"Deployed {new_sha}. Restart containers to apply changes.",
            "ref": ref,
            "sha": new_sha,
            "backup_created": backup_created,
            "note": "Run 'docker compose up -d --build' to rebuild with new code"
        }
