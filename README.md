...
- **130+ Language Support**: Powered by Google Translate API with intelligent caching. **Complete translation coverage**: UI strings, service categories, status labels, filter buttons, priority levels, search placeholders, and user-submitted content (descriptions/comments). Translation disclaimer appears only after translations load, always in the target language.
